...
import SidebarLinks from './SidebarLinks'; // Import komponen sidebar baru
...
export default async function AdminLayout({ children }: { children: React.ReactNode; }) {
    ...
    // Gabungkan nama organisasi dan role untuk ditampilkan
        ...
        <div>
            <nav className="fixed top-0 z-50 w-full bg-gray-800 border-b border-gray-700">
                <div className="px-3 py-3 lg:px-5 lg:pl-3">
                    <div className="flex items-center justify-between">
                        ...
                        {/* Kirim displayName ke UserMenu */}
                        <UserMenu user={session.user} displayName={displayName} />
                    </div>
                </div>
            </nav>
            ...
            <aside id="logo-sidebar" className="fixed top-0 left-0 z-40 w-64 h-screen pt-20 sm:translate-x-0 -translate-x-full bg-gray-800 border-r border-gray-700">
                <div className="h-full px-3 pb-4 overflow-y-auto bg-gray-800">
                    {/* Ganti <ul> lama dengan komponen sidebar baru */}
                    <SidebarLinks />
                </div>
            </aside>

            <div className="p-4 sm:ml-64"><div className="mt-14">{children}</div></div>
        </div>
    );
}